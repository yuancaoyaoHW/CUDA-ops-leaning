"""Benchmark Triton axpy vs PyTorch."""
...
def main():
    ...
    for n in [1024, 4096, 65536, 1_048_576, 16_777_216]:
        ...
        triton_axpy(x, y, alpha=alpha)
        alpha * x + y
        torch.cuda.synchronize()

        t_triton = bench(triton_axpy, (x, y, alpha))
        t_torch = bench(lambda a, b, c: c * a + b, (x, y, alpha))
    ...
    path = write_benchmark_json("axpy", results, extra={"alpha": alpha, "device": device_name})
    print()
